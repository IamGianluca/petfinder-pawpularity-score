# full credit to: https://github.com/adrianodennanni/multiresolution-cnn
...
def calc_pad_sz(
    ...
    if not dim_by_convs:
        ...
        pad_sz = [
            (left_pad, right_pad)
            for (left_pad, right_pad) in zip(left_pads, right_pads)
        ]
...
def set_scale_and_out_sz(
    ...
    if scale_factors is None and out_shape is None:
        raise ValueError(
            "either scale_factors or out_shape should be " "provided"
        )
...
def apply_antialiasing_if_needed(
    ...
    cur_interp_method = lambda arg: scale_factor * interp_method(
        scale_factor * arg
    )
...
def resizeright(img: Image, sz: int) -> Image:
    ...
    resized_img_array = img.detach().cpu().numpy().astype(np.uint8)
    # random_array = np.random.random_sample(content_array.shape) * 255
    # random_array = random_array.astype(np.uint8)

    img = Image.fromarray(resized_img_array)
    return img
