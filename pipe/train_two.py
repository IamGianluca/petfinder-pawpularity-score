...
def main():
    cfg = load_cfg(fpath=str(constants.cfg_fpath), cfg_name="train_two")
    ...
    if is_crossvalidation:
        fpath = constants.metrics_path / f"model_two.json"
        save_metrics(
            fpath=fpath,
            metric=cfg.metric,
...
def save_predictions(cfg: OmegaConf, preds: List[List]):
    ...
    with open(f"preds/model_two_fold{cfg.fold}.npy", "wb") as f:
        np.save(f, preds)
