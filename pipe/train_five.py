...
def train_one_fold(cfg: omegaconf.DictConfig, logger) -> Tuple:
    ...
    train_image_paths, train_targets = utils.get_image_paths_and_targets(
        df=df_train, cfg=cfg, target_format="multiclass_classification"
    )
    val_image_paths, val_targets = utils.get_image_paths_and_targets(
        df=df_val, cfg=cfg, target_format="multiclass_classification"
    )
    ...
    model = learner.ImageMulticlassClassifier(
        in_channels=3,
        num_classes=10,
    ...
    preds_list = [i for a in preds for i in a]
...
if __name__ == "__main__":
    cfg = load_cfg(fpath=str(constants.cfg_fpath), cfg_name=f"train_five")
    train(cfg)
